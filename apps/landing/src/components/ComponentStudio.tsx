  ...
  return (
    <div className="relative min-h-screen bg-[#07060d] text-[#f3f1ff] flex flex-col items-center px-4 sm:px-6 lg:px-8 pt-10 pb-24 overflow-hidden selection:bg-white/20 font-sans">
      ...
      <div className="relative z-10 w-full max-w-4xl min-h-[480px] rounded-[32px] bg-white/[0.04] backdrop-blur-2xl backdrop-saturate-150 border border-white/[0.12] shadow-[0_32px_90px_rgba(0,0,0,0.5),inset_0_1px_0_rgba(255,255,255,0.18)] p-6 sm:p-10 flex flex-col items-center justify-center transition-all duration-300">
        ...
        {/* VIEW 5: TOP-UP MODAL */}
        {activeTab === "modal" && (
          <div className="w-full max-w-md p-7 rounded-[28px] bg-white/[0.07] backdrop-blur-2xl border border-white/[0.18] shadow-[0_24px_70px_rgba(0,0,0,0.5),inset_0_1px_0_rgba(255,255,255,0.25)] space-y-5 animate-in fade-in duration-300">
            <div className="flex items-center justify-between">
              <div className="flex items-center gap-2.5">
                <div
                  className="w-8 h-8 rounded-xl flex items-center justify-center text-white shadow-md border border-white/20"
                  style={{ backgroundColor: accent.hex }}
                >
                  <Zap className="w-4 h-4" />
                </div>
                <div>
                  <h4 className="text-sm font-bold text-white">Doładuj Kredyty AI</h4>
                  <p className="text-[11px] text-white/50">Bezpieczny checkout przez Stripe</p>
                </div>
              </div>
              <span className="text-xs font-mono text-[#5effa8] font-bold px-3 py-1 rounded-full bg-white/[0.08] border border-white/[0.14]">
                {balance} cr
              </span>
            </div>
            ...
            {/* Provider Picker */}
            <div>
              <label className="block text-[11px] font-mono uppercase tracking-wider text-white/50 mb-2">Model Provider</label>
                ...
                {[
                  ...
                  <button
                    ...
                    className={`flex items-center gap-2 p-2.5 rounded-xl border text-xs font-medium transition-all cursor-pointer ${
                      selectedModel === p.id
                        ? "bg-white/[0.16] text-white border-white/[0.3] shadow-md"
